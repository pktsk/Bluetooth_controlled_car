...
//added by me
  ...
  text("Accelerometer: \n" + 
    ...
    char signX = nfp(accelerometerX, 1, 3).charAt(0);
    char signY = nfp(accelerometerY, 1, 3).charAt(0);
    
    //if(signX == '+' || signY == '+'){
    
    resultY = nfp(accelerometerY, 1, 3).charAt(1) - '0';
    resultX = nfp(accelerometerX, 1, 3).charAt(1) - '0';
    
    if((signX == '+' || signX == '-') && (signY =='-') && (resultX < 2) && (resultY > 2)){
      bt.broadcast(up);
    }
    else if((signX == '+' || signX == '-') && (signY =='+') && (resultX < 2) && (resultY > 2)){
    bt.broadcast(down);
    }
    else if((signX == '-') && (resultX > 2)){
    bt.broadcast(right);
    }
    else if((signX == '+') && (resultX > 2)){
    bt.broadcast(left);
